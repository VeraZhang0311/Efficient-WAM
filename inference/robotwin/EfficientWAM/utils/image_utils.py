...
def _require_decord():
    """Import decord lazily so single-frame inference does not require it at startup."""
    try:
        from decord import VideoReader, cpu
    except ImportError as exc:
        raise ImportError(
            "decord is required for video-loading utilities in EfficientWAM. "
            "Install decord, or avoid calling video-loading helpers during image-only inference."
        ) from exc
