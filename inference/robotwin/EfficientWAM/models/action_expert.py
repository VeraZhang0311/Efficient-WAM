# Action Expert model for EfficientWAM video-action training.
# DiT-style action backbone conditioned on frozen compact WAN features.
...
from ..third_party.wan.modules.model import WanRMSNorm, WanLayerNorm, sinusoidal_embedding_1d, rope_apply
...
class ActionExpertConfig:
    ...
    num_layers: int = 30             # Number of action layers
    ...
    video_feature_dim: int = 3072    # Dimension of features from the compact WAN backbone
...
class StateActionEncoder(nn.Module):
    ...
    def forward(self, state_tokens: torch.Tensor, action_tokens: torch.Tensor, registers: torch.Tensor = None) -> torch.Tensor:
        ...
        state_tokens = state_tokens.to(device=state_ref.device, dtype=state_ref.dtype)
        action_tokens = action_tokens.to(device=action_ref.device, dtype=action_ref.dtype)
        ...
        if registers is not None:
            registers = registers.to(device=encoded.device, dtype=encoded.dtype)
        ...
        pos_embedding = self.pos_embedding[:, :seq_len, :].to(device=encoded.device, dtype=encoded.dtype)
        encoded = encoded + pos_embedding
...
class ActionExpertBlock(nn.Module):
    ...
    action tokens to the head space of the frozen WAN backbone.
...
class ActionDecoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, time_emb: torch.Tensor) -> torch.Tensor:
        ...
        head_weight = self.action_head[0].weight
        return self.action_head(z.to(device=head_weight.device, dtype=head_weight.dtype))
...
class ActionExpert(nn.Module):
    ...
    def __init__(self, config: ActionExpertConfig, wan_config: dict = None):
        ...
        # EfficientWAM always conditions action prediction on the current robot state.
