...
class CompactWANModel(nn.Module):
    ...
    def extract_multiscale_hidden_features(
        self,
        condition_latent: torch.Tensor,
        future_latent: torch.Tensor,
        timestep: torch.Tensor,
        ...
        return self.video_model.get_multiscale_layer_features(
    ...
    def forward_multiscale_with_features(
        ...
        if layer_indices is None:
            layer_indices = sorted(set(self.config.hidden_anchor_layers + self.config.motion_anchor_layers))
        features = self.extract_multiscale_hidden_features(
            condition_latent=condition_latent,
            future_latent=future_latent,
            timestep=timestep,
            text_embeddings=text_embeddings,
