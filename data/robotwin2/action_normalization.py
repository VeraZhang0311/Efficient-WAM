...
def normalize_action_normalization_config(config: Optional[Mapping[str, Any]]) -> Dict[str, Any]:
    """Return a stable action normalization config for checkpoint metadata."""
    merged = dict(DEFAULT_ACTION_NORMALIZATION)
    if config:
        merged.update(dict(config))
    merged["enabled"] = bool(merged.get("enabled", False))
    merged["type"] = str(merged.get("type", "mean_std"))
    if merged.get("stats_path") is None:
        merged["stats_path"] = ""
    else:
        merged["stats_path"] = str(merged.get("stats_path", ""))
    return merged
...
class RoboTwinQposNormalizer:
    ...
    def dim(self) -> Optional[int]:
        return int(self._mean.numel()) if self._mean is not None else None
