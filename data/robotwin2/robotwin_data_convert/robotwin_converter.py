...
Target: EfficientWAM-compatible format with metas, videos, and qpos
...
class T5EmbeddingProcessor:
    ...
    def _init_encoder(self):
        ...
        if self._encoder is None:
            ...
            wan_module_path = os.path.join(script_dir, '..', '..', '..', 'third_party')
...
def process_t5_batch(args):
    ...
    logger.info(f"Starting T5 batch on {processor.device}: {total} meta files")
    
    for index, (meta_path, t5_path) in enumerate(meta_files, start=1):
        ...
        if index == total or index % progress_interval == 0:
            ...
            logger.info(
                f"T5 batch progress on {processor.device}: "
                f"{index}/{total} processed, {successful} successful"
            )
    return results
...
class RoboTwinConverter:
    """
    Converter for RoboTwin datasets to EfficientWAM-compatible format
    """
    ...
    def episode_outputs_complete(self, video_path: Path, qpos_path: Path, meta_path: Path) -> bool:
        """Check whether an existing episode output is complete enough to skip."""
        if not all(path.exists() and path.stat().st_size > 0 for path in [video_path, qpos_path, meta_path]):
            return False
        
        video_capture = cv2.VideoCapture(str(video_path))
        try:
            if not video_capture.isOpened() or int(video_capture.get(cv2.CAP_PROP_FRAME_COUNT)) <= 0:
                logger.warning(f"Existing video is not readable, will regenerate: {video_path}")
                return False
        finally:
            video_capture.release()
        
        try:
            torch.load(qpos_path, map_location="cpu")
        except Exception as e:
            logger.warning(f"Existing qpos file is not readable, will regenerate: {qpos_path} ({e})")
            return False
    ...
    def process_episode(
        ...
        try:
            ...
            if self.config.get('skip_existing', False):
                if self.episode_outputs_complete(video_path, qpos_path, meta_path):
                    logger.info(f"Skipping existing episode {episode_id}")
                    return True
    ...
    def _find_hdf5_files(self, dataset_dir: Path) -> List[Path]:
        """Find episode HDF5 files under a RoboTwin demo directory."""
    ...
    def scan_dataset(self, source_root: str) -> Dict[str, Dict[str, List[Path]]]:
        ...
        Returns:
            Nested dictionary with structure: {target_subset: {task: [episode_paths]}}
        """
        dataset_structure: Dict[str, Dict[str, List[Path]]] = {}
        source_path = Path(source_root)
        demo_type_mapping = self.config.get('demo_type_mapping', {})
        normalized_splits = set(demo_type_mapping.values())

        def add_demo(target_subset: str, task_name: str, demo_dir: Path) -> bool:
            hdf5_files = self._find_hdf5_files(demo_dir)
            if not hdf5_files:
                return False
            task_bucket = dataset_structure.setdefault(target_subset, {}).setdefault(task_name, [])
            task_bucket.extend(hdf5_files)
            return True
        
        for first_level_dir in sorted(path for path in source_path.iterdir() if path.is_dir()):
            # Preferred RoboTwin2 layout:
            # source_root/{task_name}/{demo_type}/data/*.hdf5
            matched_task_layout = False
            for demo_dir in sorted(path for path in first_level_dir.iterdir() if path.is_dir()):
                target_subset = demo_type_mapping.get(demo_dir.name)
                if target_subset is None:
                    continue
                matched_task_layout |= add_demo(target_subset, first_level_dir.name, demo_dir)

            if matched_task_layout:
                continue

            # Compatibility layout:
            # source_root/{demo_type}/{task_name}/data/*.hdf5
            mapped_subset = demo_type_mapping.get(first_level_dir.name)
            if mapped_subset is not None:
                for task_dir in sorted(path for path in first_level_dir.iterdir() if path.is_dir()):
                    add_demo(mapped_subset, task_dir.name, task_dir)
                continue

            # Already-normalized layout:
            # source_root/{clean|randomized}/{task_name}/data/*.hdf5
            if not demo_type_mapping or first_level_dir.name in normalized_splits:
                for task_dir in sorted(path for path in first_level_dir.iterdir() if path.is_dir()):
                    add_demo(first_level_dir.name, task_dir.name, task_dir)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Convert RoboTwin dataset to EfficientWAM-compatible format"
    )
    parser.add_argument(
        "--config", 
        type=str, 
